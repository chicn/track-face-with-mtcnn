...
Pnet::Pnet(){
    ...
    string filename = "../mtcnn/Pnet.txt";
...
Rnet::Rnet(){
    ...
    string filename = "../mtcnn/Rnet.txt";
    readData(filename, dataNumber, pointTeam);
...
Onet::Onet(){
    ...
    string filename = "../mtcnn/Onet.txt";
    readData(filename, dataNumber, pointTeam);
...
void mtcnn::findFace(const Mat &image){
    faceboxes.clear();
    struct orderScore order;
    int count = 0;
    ...
    for(vector<struct Bbox>::iterator it=thirdBbox_.begin(); it!=thirdBbox_.end();it++){
        if((*it).exist){
            ...
            faceboxes.push_back(cv::Rect(Point((*it).y1, (*it).x1), Point((*it).y2, (*it).x2)));
...
std::vector<cv::Rect2d> mtcnn::getFaceBoxes() {
    return faceboxes;
}